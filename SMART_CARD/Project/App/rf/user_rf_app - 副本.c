...
void GPIO_Configuration(void)
{
	...
	//B口 推挽输出
	GPIO_InitStructure.GPIO_Pin =   GPIO_PIN_START_KEY|GPIO_PIN_BREAK_OUT |GPIO_PIN_BEEP|  GPIO_PIN_LED|
...
static u8 UhfRevData_V7(u8 *uhfdata, u16 checktime) 
{
	...
	fallingvalue += (UHFPLUSE_V7 >> 1);
	while(TIM_GetCounter(TIM2) < fallingvalue && TIM2_counting_now_flag == TRUE) 	//延时1/2T
	{
	}

	TIM_SetCounter(TIM2, 0);
	index = 2;
	uhfbitstate = 0;	//数据帧起始电平
	fallingvalue = 0;
	risingvalue = 0;
	do{
		while(READ_UHF_IN == (BitAction)uhfbitstate && TIM2_counting_now_flag == TRUE) 
		{
		}
		
		risingvalue = TIM_GetCounter(TIM2) + UHFFILTER;
		while(TIM_GetCounter(TIM2) < risingvalue && TIM2_counting_now_flag == TRUE) 
		{
		}
		...
		risingvalue = TIM_GetCounter(TIM2);
		uhf_pulse_l = risingvalue - fallingvalue;	//获得一个脉冲的时长
		if(risingvalue > 60000) 				//检测波形超过130ms，计数器清零，继续
		{
			risingvalue = 0;
		...
		if(uhf_pulse_l < UHFMAST_V7) 
		{
			...
			if(index & 0x01)
			{
				uhfdata[index >> 4] |= uhfbitstate << (7 - ((index & 0x0F) >> 1));	//低字节，高位，在前
			}
		}
		else 
		{
			index += 2;
			uhfdata[index >> 4] |= uhfbitstate << (7 - ((index & 0x0F) >> 1));
		}
...
{	
	...
	switch(key) 
	{
		...
		case REMOTE_KEY_STOP:
			DriveAlarm(1, 100);
			if((READ_ON_LINE==1)&&(on_change_flag==0))
			{
...
static void OnStudyRemote(void) 
{
	...
	{ 
		...
		if((( (LastTime-InitTime) > 8000) /*||(READ_STUDYKEY == Bit_RESET)*/) 	//持续接收时间10s
		    ||(key_sn>=temp)
		  )
		{
...
void IWDG_Configuration(void)
{
	...
	IWDG_SetReload(400);							// 设定独立看门狗计数器的计数值(0x000~0xFFF;0~4095)，复位时间为2.56s
...
void RfTask(void *p_arg)
{
	...
	while(1)
	{
		...
		#else
		if(user_get_os_time()-last_adc_time>T_500MS)
		{
			last_adc_time=user_get_os_time();
		...
		if(user_get_os_time()-last_time>T_5S)
		{
