...
*解密函数
************************************************/
void AesDecryptData(uint8* databuffer,uint32 datalen)
{
	uint32 i = 0;

	for(i = 0; i < datalen; i += 16)
		aes_decrypt( &ctx_aes, &databuffer[i],&databuffer[i] );
}
/***********************************************
*加密函数
************************************************/
void AesEncryptData(uint8* databuffer,uint32 datalen)
{
	uint32 i = 0;
	for(i = 0; i < datalen; i += 16)
		aes_encrypt( &ctx_aes, &databuffer[i],&databuffer[i] );
}
/***********************************************
*初始化秘钥信息
************************************************/
void SetAesKey(unsigned char CK[16])
{
    aes_set_key(&ctx_aes, CK, 128 );
}
extern unsigned int time_now_to_uint32(void);
/***********************************************
*随机数生成函数
************************************************/
void get_authentication_rand(unsigned char* p_rand)
{
	unsigned char	i;
	unsigned char	temp;

	srand(time_now_to_uint32());
	for( i = 0 ; i < 8 ; i++ )
	{
		temp = rand() & 0xff;
		p_rand[i*2+0] = ~temp;
